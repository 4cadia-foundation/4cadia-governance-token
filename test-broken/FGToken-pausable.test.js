...
contract('FGToken', accounts => {
  ...
  describe('pause', () => {
    ...
    it('should return not pausable role', async () => {
      await truffleAssertions.fails(this.token.pause({ from: accounts[4] }), 'CEORole: onlyCEO');
    });
  ...
  describe('unpause', () => {
    ...
    it('should return does not have the Pauser role', async () => {
      await this.token.pause({ from: accountCEO });
      await truffleAssertions.fails(this.token.unpause({ from: accounts[3] }), 'CEORole: onlyCEO');
    });
