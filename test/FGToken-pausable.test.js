...
contract('FGToken', accounts => {
  ...
  describe('pause', () => {
    ...
    it('should return not pausable role', async () => {
      await truffleAssertions.fails(this.token.pause({ from: accounts[1] }), 'CEORole: caller does not have the CEO role');
    });
  ...
  describe('unpause', () => {
    ...
    it('should return does not have the Pauser role', async () => {
      await this.token.pause();
      await truffleAssertions.fails(this.token.unpause({ from: accounts[1] }), 'CEORole: caller does not have the CEO role');
    });
